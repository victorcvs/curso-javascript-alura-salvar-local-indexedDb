                ...
                _createClass(DateHelper, null, [{
                    ...
                    value: function textoParaData(texto) {
                        ...
                        if (!/^\d{4}-\d{2}-\d{2}$/.test(texto)) {
                            // Regex validação de máscara numérica
                            // O ˆ indica "começando com " e o $ "terminando com". 
                            throw new Error('Data deve estar no formato AAAA-MM-DD');
                        }

                        return new (Function.prototype.bind.apply(Date, [null].concat(_toConsumableArray(texto // spread operator -> permite passar array como parâmetro casando os indices
                        .split('-') // separa a data em array usando o '-' como separador
                        .map(function (item, indice) {
                            return item - indice % 2;
                        })))))(); // formata o array com função que subtrai 1 dos elementos ìmpares (mod 2)
                    }
                }, {
                    key: 'dataParaTexto',
